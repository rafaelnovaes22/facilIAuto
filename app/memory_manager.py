...
from app.memory_models import Base, Conversation, ConversationMessage, ConversationContext, UserSession
...
class ConversationMemoryManager:
    ...
    def _update_primary_agent(self, session: Session, conversation: Conversation, agent_used: str):
        ...
        ).group_by(ConversationMessage.agent_used).all()
    ...
    def persist_conversation_result(
        ...
        try:
            ...
            self._extract_and_persist_context(
                conversation_id, user_message, agent_used
            )
    ...
    def _extract_and_persist_context(
        ...
        try:
            ...
            brands = ["toyota", "honda", "volkswagen", "hyundai", "chevrolet", "ford", "nissan"]
            for brand in brands:
    ...
    def get_conversation_analytics(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            total_messages = session.query(ConversationMessage).filter(
            ...
            car_popularity = session.query(
                Conversation.carro_id,
                func.count(Conversation.id).label('count')
            ).filter(
                Conversation.started_at >= since_date
            ).group_by(Conversation.carro_id).order_by(desc('count')).limit(10).all()
