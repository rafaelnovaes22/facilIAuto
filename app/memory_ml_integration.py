    ...
    def get_recent_ml_feedback(self, days: int = 30) -> List[Dict]:
        ...
        try:
            with self.memory.Session() as session:
                ...
                results = session.query(ConversationContext).filter(
                    ConversationContext.context_type.in_(["ml_feedback", "ml_training"]),
    ...
    def get_ml_performance_stats(self) -> Dict[str, Any]:
        ...
        try:
            with self.memory.Session() as session:
                ...
                results = session.query(ConversationContext).filter(
                    ConversationContext.context_type == "recommendation"
                ).limit(100).all()
