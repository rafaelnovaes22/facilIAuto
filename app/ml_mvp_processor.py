...
class IntegratedMLCollector:
    ...
    def collect_from_conversation(
        ...
        if conversation_id:
            conversation, messages = self.memory.get_conversation_history(conversation_id)
            if not conversation:
                return
    ...
    def _extract_enhanced_features(self, carro: Dict) -> Dict:
        ...
        features = {
            ...
            "marca_normalizada": AdvancedBrandMatcher().normalize_brand(carro.get("marca", "")),
...
class SmartMVPModel:
    ...
    def prepare_smart_features(
        ...
        if resultado_busca.get("pontuacoes"):
            ...
            features.extend([
                ...
                score_data.get("score", 0) * 0.15,  # Aproximação para score_categoria
                score_data.get("score", 0) * 0.1,  # Aproximação para score_economia
                score_data.get("score", 0) * 0.1,  # Aproximação para score_opcionais
            ])
        else:
            features.extend([0, 0, 0, 0, 0])
        
        # 2. Features do Uso Principal (aproveitar cálculos complexos)
        uso_score, uso_razoes, uso_fortes = self.uso_matcher.calcular_score_uso_principal(
            questionario, carro
        )
        ...
        if conversation_id:
            ...
            if conversation_data:
                features.extend([
                    ...
                    1 if conversation_data.primary_agent else 0,  # Tem agente principal
        ...
        features.append(questionario.orcamento_maximo / 200000 if questionario.orcamento_maximo else 0.5)
        features.append(1 if questionario.primeiro_carro else 0)
        features.append(questionario.pessoas_familia / 7 if questionario.pessoas_familia else 0.5)
        features.append(questionario.km_por_mes / 3000 if questionario.km_por_mes else 0.5)
        
        # 8. Opcionais importantes (aproveitar análise existente)
        opcionais_text = " ".join(carro.get("opcionais", [])).lower()
        important_features = [
            "ar condicionado", "direção", "airbag", "abs",
            "android auto", "apple carplay", "sensor", "câmera",
            "automático", "couro", "teto solar", "rodas"
        ]
    ...
    def _get_feature_names(self) -> List[str]:
        ...
        """
        return [
            "Score Total", "Score Preço", "Score Categoria", "Score Economia", "Score Opcionais",
            "Uso Score", "Razões Count", "Pontos Fortes", "Brand Valid", "Brand Confidence",
            "Messages", "Has Agent", "Interactions",
            "Preço", "Idade", "KM", "Consumo", "Cilindrada", "Porta-malas", "Lugares", "Segurança",
            "É SUV", "É Sedan", "É Hatch", "É Flex", "É Automático",
            "Orçamento", "Primeiro Carro", "Família Size", "KM/mês",
            "Ar Condicionado", "Direção", "Airbag", "ABS", "Android Auto", "Apple CarPlay",
            "Sensor", "Câmera", "Automático", "Couro", "Teto Solar", "Rodas"
        ]
    ...
    def save_model(self):
        ...
        model_data = {
            ...
            'timestamp': datetime.now().isoformat(),
            'metrics': {
                'n_samples': len(self.model.estimators_[0].tree_.value) if self.model else 0,
...
class SuperHybridProcessor:
    ...
    def processar_recomendacao_completa(
        ...
        resultado_busca = calcular_scores_compatibilidade(estado_busca)
        busca_score = resultado_busca["pontuacoes"][0]["score"] if resultado_busca.get("pontuacoes") else 0
    ...
    def get_comprehensive_stats(self) -> Dict[str, Any]:
        ...
        ml_performance = {
            "avg_confidence": self.ml_model.confidence_score if hasattr(self.ml_model, 'confidence_score') else 0.0,
