...
def test_imports():
    ...
    try:
        ...
        from app.chatbot_models import TipoAgente, PerguntaChatbot, RespostaChatbot
        from app.langgraph_chatbot_state import ChatbotState, AgentType
...
def test_graph_initialization():
    ...
    try:
        ...
        print_step(f"Singleton pattern: {'OK' if is_singleton else 'FALHOU'}", "ok" if is_singleton else "error")
...
def test_agent_routing(chatbot_graph, carro_data):
    ...
    for i, test_case in enumerate(test_cases, 1):
        try:
            ...
            roteamento_correto = (
                agente_usado == test_case['agente_esperado'] or
                (test_case['agente_esperado'] == "finalizer" and confianca < 0.3) or
                confianca >= 0.3
            )
    ...
    print_step(f"Testes de roteamento: {success_count}/{len(test_cases)} sucessos", 
...
def test_response_quality(chatbot_graph, carro_data):
    ...
    try:
        ...
        print_step(f"    Elementos técnicos encontrados: {elementos_encontrados}/{len(elementos_esperados)}", 
        ...
        print_step(f"    Elementos financeiros encontrados: {elementos_fin_encontrados}/{len(elementos_financeiros)}", 
...
def test_debug_mode(chatbot_graph, carro_data):
    ...
    try:
        ...
        if "final_result" in debug_info:
            ...
            print_step(f"Resultado final: agente={final_result.get('agente_usado', 'N/A')}", "ok")
