...
def test_memory_imports():
    ...
    try:
        ...
        from app.memory_models import Conversation, ConversationMessage, ConversationContext, UserSession
...
def test_message_persistence(memory_manager, conversation_id):
    ...
    try:
        test_messages = [
            ...
            ("assistant", "O consumo é de 12.5 km/l na cidade e estrada", "tecnico", 0.9),
            ("user", "E o financiamento, como funciona?"),
            ("assistant", "Posso simular várias opções de financiamento para você", "financeiro", 0.8)
        ]
...
def test_analytics_system(memory_manager):
    ...
    try:
        ...
        print_step(f"Total de conversas (7 dias): {analytics.get('total_conversations', 0)}", "ok")
        print_step(f"Total de mensagens (7 dias): {analytics.get('total_messages', 0)}", "ok")
        print_step(f"Média de mensagens por conversa: {analytics.get('avg_messages_per_conversation', 0)}", "ok")
...
def test_similar_conversations(memory_manager):
    ...
    try:
        ...
        for i, (conv, messages) in enumerate(similar_convs):
            print_step(f"  Conversa {i+1}: {conv.total_messages} mensagens, agente: {conv.primary_agent}", "ok")
