...
class TestFamiliaCriteriosE2E:
    ...
    def carro_ideal_familia(self):
        ...
        return {
            ...
            'preco': 65000,
            'opcionais': [
                'ar_condicionado', 'airbag_multiplos', 'abs', 'controle_estabilidade', 
                'vidros_eletricos', 'travas_eletricas', 'isofix', 'camera_re', 
                'bancos_rebatraveis', 'apoio_braco', 'porta_objetos'
            ],
            'combustivel': 'flex',
            'seguranca': 5,
            'revisado': True,
    ...
    async def test_espaco_passageiros_conforto_e2e(self, questionario_familia_completo, carro_ideal_familia):
        ...
        espaco_razoes = [r for r in razoes if any(palavra in r.lower() for palavra in ['espaço', 'família', 'pessoas', 'conforto'])]
    ...
    async def test_praticidade_uso_familiar_e2e(self, questionario_familia_completo, carro_ideal_familia):
        ...
        praticidade_razoes = [r for r in razoes if any(palavra in r.lower() for palavra in ['praticidade', 'facilidades', 'porta-malas'])]
    ...
    async def test_sugestoes_especificas_familia_e2e(self, questionario_familia_completo):
        ...
        familia_sugs = [s for s in sugestoes if any(w in s.lower() for w in [
            'família', 'segurança', 'airbags', 'isofix', 'criança', 'custo', 'atividades', 'esportivas'
        ])]
...
async def test_performance_criterios_familia_e2e():
    ...
    carro = {
        ...
        'cilindrada': 1.8, 'consumo': 10, 'porta_malas_litros': 480,
        'pessoas_transportar': 5, 'preco': 65000, 'combustivel': 'flex', 
        'seguranca': 5, 'revisado': True, 'km': 35000,
