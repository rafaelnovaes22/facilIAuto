...
class TestViagemCriteriosE2E:
    ...
    def carro_ideal_viagem(self):
        ...
        return {
            ...
            'pessoas_transportar': 5,
            'opcionais': [
                'ar_condicionado', 'climatizador_automatico', 'bancos_couro', 
                'gps_integrado', 'android_auto', 'controle_estabilidade', 
                'abs', 'airbag_multiplos', 'direcao_assistida', 'piloto_automatico'
            ],
            'combustivel': 'flex',
            'seguranca': 5,
            'revisado': True,
    ...
    async def test_desempenho_seguranca_estrada_e2e(self, questionario_viagem_completo, carro_ideal_viagem):
        ...
        desempenho_razoes = [r for r in razoes if any(palavra in r.lower() for palavra in ['desempenho', 'estrada', 'motor', 'potente'])]
    ...
    async def test_espaco_capacidade_carga_e2e(self, questionario_viagem_completo, carro_ideal_viagem):
        ...
        espaco_razoes = [r for r in razoes if any(palavra in r.lower() for palavra in ['capacidade', 'espaço', 'pessoas', 'porta-malas'])]
    ...
    async def test_tecnologia_entretenimento_e2e(self, questionario_viagem_completo, carro_ideal_viagem):
        ...
        tech_razoes = [r for r in razoes if any(palavra in r.lower() for palavra in ['tecnologia', 'gps', 'entretenimento', 'navegação'])]
    ...
    async def test_sugestoes_especificas_viagem_e2e(self, questionario_viagem_completo):
        ...
        viagem_sugs = [s for s in sugestoes if any(w in s.lower() for w in [
            'viagem', 'estrada', 'conforto', 'turismo', 'road', 'gps', 'climatiz'
        ])]
    ...
    async def test_descricao_viagem_atualizada_e2e(self):
        ...
        elementos = ['conforto', 'desempenho', 'espaço', 'economia', 'tecnologia', 'subsegmentos']
...
async def test_performance_criterios_viagem_e2e():
    ...
    carro = {
        'marca': 'Honda', 'modelo': 'CR-V', 'categoria': 'SUV',
        'cilindrada': 1.8, 'consumo_estrada': 12, 'porta_malas_litros': 480,
        'pessoas_transportar': 5, 'combustivel': 'flex', 'seguranca': 5,
        'revisado': True, 'km': 45000,
        'opcionais': ['ar_condicionado', 'climatizador_automatico', 'bancos_couro', 
                     'gps_integrado', 'android_auto', 'controle_estabilidade', 
                     'abs', 'airbag_multiplos', 'direcao_assistida']
    }
