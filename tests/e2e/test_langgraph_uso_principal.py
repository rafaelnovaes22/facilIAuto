...
class TestLangGraphUsoPrincipalE2E:
    ...
    def test_roteamento_automatico_uso_principal(self, chatbot_graph, carro_teste):
        ...
        for pergunta in perguntas_uso:
            ...
            assert resultado["agente"] == "uso_principal" or \
                   "uso_principal" in str(resultado.get("resposta", "")), \
            ...
            assert any(termo in resposta.lower() for termo in ["uso", "adequado", "ideal", "recomend"]), \
    ...
    def test_analise_detalhada_uso_urbano(self, chatbot_graph, carro_teste):
        ...
        criterios_urbanos = ["compacto", "econômico", "manobra", "estacionamento", "economia"]
        assert any(criterio in resposta.lower() for criterio in criterios_urbanos), \
            "Resposta deve conter critérios específicos de uso urbano"
        
        # Deve ter avaliação visual
        avaliacoes = ["🌟", "👍", "⚖️", "✅", "⚠️"]
        assert any(emoji in resposta for emoji in avaliacoes), \
            "Resposta deve conter avaliação visual"
        
        # Para Hatch, deve ser positivo para uso urbano
        assert "excelente" in resposta.lower() or "adequado" in resposta.lower() or \
               "ideal" in resposta.lower(), \
    ...
    def test_integracao_uso_matcher(self, carro_teste):
        ...
        assert "família" in razoes_texto or "segur" in razoes_texto, \
    ...
    def test_regressao_agentes_existentes(self, chatbot_graph, carro_teste):
        ...
        for pergunta, agente_esperado in perguntas_outros_agentes:
            ...
            agente_selecionado = resultado.get("agente_selecionado", "")
