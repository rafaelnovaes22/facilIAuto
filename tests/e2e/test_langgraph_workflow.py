...
class TestLangGraphWorkflowE2E:
    ...
    def sample_car_data(self):
        ...
        return {
            ...
            "quilometragem": 15000,
            "cor": "Branco",
            "opcionais": ["Ar Condicionado", "Central Multimídia", "Sensor de Estacionamento"]
        }
    ...
    async def test_complete_langgraph_workflow_api(self, client, sample_car_data):
        ...
        required_fields = ["resposta", "agente", "conversation_id", "confianca", "sugestoes_followup"]
        ...
        assert response_data["agente"] == expected_agent, f"Esperado agente '{expected_agent}', recebido '{response_data['agente']}'"
        ...
        assert any(term in resposta.lower() for term in ["km/l", "litro", "economia"]), "Resposta deve incluir unidades/termos técnicos"
    ...
    async def test_performance_benchmarks(self, client, sample_car_data):
        ...
        assert avg_response_time <= max_avg_response_time, f"Tempo médio muito alto: {avg_response_time:.0f}ms (máximo: {max_avg_response_time}ms)"
        assert max_response_time_actual <= max_response_time, f"Tempo máximo muito alto: {max_response_time_actual:.0f}ms (máximo: {max_response_time}ms)"
    ...
    async def test_error_handling_and_recovery(self, client):
        ...
        response1 = client.post("/api/chatbot/perguntar", json={
        ...
        response2 = client.post("/api/chatbot/perguntar", json={
            "carro_id": 1,
            "pergunta": "",
            "user_session_id": f"error_test_{uuid.uuid4()}"
        })
        ...
        response3 = client.post("/api/chatbot/perguntar", json={
